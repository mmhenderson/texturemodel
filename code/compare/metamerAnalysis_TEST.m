...
disp('len of Cx0.scale')
disp(length(Cx0.scale))
disp('len of C0.scale')
disp(length(C0.scale))
...
for pp=1:6
    fprintf('  Weighted %s in original image: %.8f\n',names{pp},statg0(pp,1))
end


mag_mean_matrix = zeros(Nsc, Nor);
ii=1;
for sc=1:Nsc
    for or=1:Nor
        ii=ii+1;
        mag_mean_matrix(sc,or) = magMeans0.band{ii}(1);
    end
end
fprintf('Mean of feature magnitudes in each band [nSF x nOri]\n')
disp(mag_mean_matrix)

% Going to print some more stuff here - NOTE that the scales go in reverse order in my code relative to this matlab code.
% But the number of pixels per image at each scale should clarify which scale is which.

sc2print=5;
fprintf('lowpass band, npix=%d\n',size(m.scale{sc2print}.maskMat,2))
fprintf('  Weighted skew in lowpass recon: %.8f\n',skew0p.scale{sc2print}(1))
fprintf('  Weighted kurtosis in lowpass recon: %.8f\n',kurt0p.scale{sc2print}(1))
fprintf('  Lowpass recon autocorrelation: %.8f\n',acr.scale{sc2print}.mask{1}(1,2))  
fprintf('  Real/spatially shifted lowpass within scale cross-correlation: %.8f\n',Cr0.scale{sc2print}.mask{1}(1,2))

ori1 = 1;
ori2 = 2;
for sc2print = [4,3,2,1]
    fprintf('sc=%d, npix=%d\n',sc2print, size(m.scale{sc2print}.maskMat,2))
    fprintf('  Weighted skew in lowpass recon: %.8f\n',skew0p.scale{sc2print}(1))
    fprintf('  Weighted kurtosis in lowpass recon: %.8f\n',kurt0p.scale{sc2print}(1))
    fprintf('  Magnitude autocorrelation: %.8f\n',ace.scale{sc2print}.ori{ori1}.mask{1}(1,2))
    fprintf('  Lowpass recon autocorrelation: %.8f\n',acr.scale{sc2print}.mask{1}(1,2))  
    fprintf('  Magnitude within scale cross-correlation: %.8f\n',C0.scale{sc2print}.mask{1}(ori1, ori2))
    fprintf('  Real within scale cross-correlation: %.8f\n',Cr0.scale{sc2print}.mask{1}(ori1, ori2))
    if sc2print<4
        fprintf('  Magnitude across scale cross-correlation: %.8f\n',Cx0.scale{sc2print}.mask{1}(ori1, ori2))
        fprintf('  Real/real across scale cross-correlation: %.8f\n',Crx0.scale{sc2print}.mask{1}(ori1, ori2))
        fprintf('  Real/imag across scale cross-correlation: %.8f\n',Crx0.scale{sc2print}.mask{1}(ori1, ori2+nor))    
    else
        fprintf('  Real/spatially shifted lowpass across scale cross-correlation: %.8f\n',Crx0.scale{sc2print}.mask{1}(ori1, ori2))
...
sc2print=1;
fprintf('highpass residual, npix=%d\n',size(m.scale{sc2print}.maskMat,2))
fprintf('  Highpass resid autocorrelation: %.8f\n',acr.vHPR.mask{1}(1,2)) 
fprintf('  Highpass resid variance: %.8f\n',vHPR0(1)) 
